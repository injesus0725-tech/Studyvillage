...
assert.ok(/app\.quit\(\)\s*;?/.test(main),'Studyvillage 창을 닫으면 앱이 종료되어야 합니다.');
assert.ok(main.includes("app.on('before-quit'"),'앱 종료 전에 교실 서버를 정리하는 처리가 필요합니다.');
assert.ok(/classroomServer\?\.listening[\s\S]*classroomServer\.close\(\)/.test(main),'앱 종료 시 내장 교실 서버를 정상적으로 닫아야 합니다.');
