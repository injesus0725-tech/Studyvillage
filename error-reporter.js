/* v0.9.7 privacy-safe automatic client error reporter.
...
(()=>{
  ...
  function makeReport(kind,message,stack='',extra={}){
    return{id:`err-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,at:now(),version:VERSION,kind:safeText(kind,80),message:safeText(message),stack:safeText(stack,5000),page:page(),online:navigator.onLine,mode:mode(),userAgent:safeText(navigator.userAgent,500),viewport:{width:innerWidth,height:innerHeight},extra,recentEvents:read(EVENT_KEY).slice(-20)};
  }
  function saveReport(kind,message,stack='',extra={}){
    const rows=read(STORAGE_KEY),report=makeReport(kind,message,stack,extra);rows.push(report);write(STORAGE_KEY,trim(rows,MAX_REPORTS));setTimeout(flush,0);return report;
  }
  async function send(report){
    const headers=window.StudyVillageAuth?.authHeaders?.()||{};
    if(!headers.Authorization)return false;
    try{
      const r=await originalFetch('/api/error-report',{method:'POST',headers:{'Content-Type':'application/json',...headers},body:JSON.stringify(report)});
      return r.ok;
    }catch{return false}
  }
  async function flush(){
    ...
    flushing=true;
    try{
      const rows=read(STORAGE_KEY),remaining=[];
      for(const report of rows){if(!(await send(report)))remaining.push(report)}
      write(STORAGE_KEY,remaining);
  ...
  window.addEventListener('online',()=>{addEvent('network','브라우저 online');flush()});
  ...
  window.fetch=async(...args)=>{
    ...
    try{
      ...
      if(!response.ok&&String(url).includes('/api/')&&!String(url).includes('/api/error-report')){
      ...
      if(String(url).includes('/api/login')&&response.ok)setTimeout(flush,100);
      return response;
    }catch(error){
      const safeUrl=String(url).split('?')[0];
      if(String(url).includes('/api/')&&!String(url).includes('/api/error-report'))addEvent('api-network-failure',`${method} ${safeUrl}`);
      throw error;
    }
  };
  ...
  window.StudyVillageErrorReporter={addEvent,saveReport,flush,download,count:()=>read(STORAGE_KEY).length,clear:()=>{write(STORAGE_KEY,[]);write(EVENT_KEY,[])}};
  addEvent('page-load',`v${VERSION}`);setInterval(flush,10000);setTimeout(flush,1000);
})();