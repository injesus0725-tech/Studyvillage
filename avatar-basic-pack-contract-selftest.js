...
const v5={hair:['hair-flame-red','hair-lavender-bob'],hat:['hat-frog-hood','hat-knight-mini'],glasses:['glasses-lightning'],outfit:['outfit-adventure-red','outfit-mage-blue'],bottom:['bottom-adventure-brown'],shoes:['shoes-trail-orange'],bag:['bag-treasure'],hand:['hand-shield','hand-crystal','hand-balloon','hand-candy-cane','hand-lollipop','hand-flower-bouquet','hand-village-flag','hand-bubble-wand','hand-lantern','hand-umbrella','hand-trophy','hand-guitar','hand-star-cookie','hand-magic-orb'],pet:['pet-fennec','pet-baby-wolf','pet-moon','pet-unicorn','pet-robot','pet-smile-cloud','pet-friendly-ghost','pet-sprout','pet-baby-seal','pet-red-panda','pet-phoenix','pet-octopus','pet-side-heart','pet-side-music','pet-side-wow','pet-side-cheer','pet-side-fire','pet-side-rainbow','pet-side-sparkle','pet-side-thumbs','pet-side-question']};
const ids=Object.values(basics).flat(),v3Ids=Object.values(v3).flat(),v4Ids=Object.values(v4).flat(),v5Ids=Object.values(v5).flat();
assert.equal(ids.length,42);assert.equal(v3Ids.length,15);assert.equal(v4Ids.length,15);assert.equal(v5Ids.length,45);assert.equal(ids.length+v3Ids.length+v4Ids.length+v5Ids.length,117,'active lightweight wardrobe must total 117 items');
...
assert.ok(secondary.includes('.sv-profile-face,.sv-exp2-face{z-index:3}')&&secondary.includes('.sv-profile-hair,.sv-exp2-hair{z-index:4}')&&secondary.includes('.sv-profile-outfit,.sv-exp2-outfit,.sv-profile-bottom,.sv-exp2-bottom,.sv-profile-shoes,.sv-exp2-shoes{z-index:5}'),'profile and expedition hair must cover the face edge while staying behind clothing');
assert.ok(css.includes('.preview-face{z-index:3!important}')&&css.includes('.preview-hair{z-index:4!important}')&&css.includes('.preview-outfit,.preview-bottom,.preview-shoes{z-index:5!important}'),'preview face, hair, and clothing must have distinct layer order');
assert.ok(css.includes('.avatar-expression{z-index:6!important}')&&css.includes('.preview-expression{z-index:6!important}'),'expression layer must remain visible');
assert.ok(css.includes('width:100%!important')&&css.includes('object-fit:contain!important')&&!css.includes('width:145%!important'),'square PNG avatar art must keep its native proportions on a square modular canvas');
...
assert.ok(!clientV5.includes("['face','얼굴']")&&!clientV5.includes("['expression','표정']"),'완성형 캐릭터 구조에서는 별도 얼굴·표정 필터를 만들지 않습니다.');
assert.ok(clientV5.includes('#student-shop-items{max-height:min(44vh,420px);overflow-y:auto'),'117-item shop list must remain bounded and scrollable');
