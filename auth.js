...
window.StudyVillageAuth=(()=>{
  ...
  function rememberName(name){const names=knownNames();names.add(name.trim().toLowerCase());localStorage.setItem(KNOWN_NAMES_KEY,JSON.stringify([...names].slice(-100)))}
  ...
  async function restoreSession(){if(!sessionToken||!sessionName)return null;if(!(await checkServer(true)))return null;try{const response=await timedFetch('/api/player/me',{headers:{Authorization:`Bearer ${sessionToken}`},cache:'no-store'});if(!response.ok){if(response.status===401)clearSession();return null}const result=await response.json();if(!result.ok||!result.player)return null;restoredPlayer=result.player;rememberName(sessionName);return{ok:true,name:sessionName,player:result.player,mode:'classroom-server'}}catch{invalidateServer();return null}}
