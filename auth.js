/* v0.9.53 authentication router.
   Classroom HTTP(S) pages stay on the classroom-server path even when a health probe is briefly blocked or delayed. */
window.StudyVillageAuth=(()=>{
  ...
  const classroomOrigin=location.protocol==='http:'||location.protocol==='https:';
  const toHex=bytes=>[...new Uint8Array(bytes)].map(b=>b.toString(16).padStart(2,'0')).join('');
  async function hashPassword(password,salt){if(!globalThis.crypto?.subtle)throw new Error('secure-context-required');return toHex(await globalThis.crypto.subtle.digest('SHA-256',encoder.encode(`${salt}:${password}`)))}
  const key=name=>prefix+name.trim().toLowerCase(),randomSalt=()=>{const bytes=new Uint8Array(16);globalThis.crypto.getRandomValues(bytes);return toHex(bytes)},read=name=>{try{return JSON.parse(localStorage.getItem(key(name))||'null')}catch{return null}};
  ...
  async function timedFetch(url,options={},timeoutMs=REQUEST_TIMEOUT_MS){const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),timeoutMs);try{return await fetch(url,{...options,signal:controller.signal,cache:options.cache||'no-store'})}finally{clearTimeout(timeout)}}
  ...
  return{login,restoreSession,checkServer,authHeaders,clearSession,logoutSession,invalidateServer,restoreSentinel:RESTORE_SENTINEL,mode:()=>serverAvailable?'classroom-server':classroomOrigin?'classroom-server':'local'};
})();
