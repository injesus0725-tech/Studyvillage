/* v0.9.54 authentication router.
   Classroom HTTP/HTTPS origins never silently fall back to a local-only account when the server is unavailable. */
window.StudyVillageAuth=(()=>{
  ...
  return{login,restoreSession,checkServer,authHeaders,clearSession,logoutSession,invalidateServer,restoreSentinel:RESTORE_SENTINEL,mode:()=>classroomOrigin||serverAvailable?'classroom-server':'local'};
})();
