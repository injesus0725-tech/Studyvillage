...
assert.ok(response.includes('window.StudyVillageCurriculumSupplementReady=Promise.all([')&&response.includes("import('./assets/curriculum-content-supplement.js")&&response.includes("import('./assets/bookmaru-variety-supplement.js")&&response.includes("import('./assets/math-curriculum-supplement.js"),'supplement loader must expose one readiness promise for every supplemental bank before student filtering');
...
for(const token of ["eligible?.(q,'bookmaru')","(set.spaces||[]).includes('bookmaru')","(q.spaces||set.spaces||[]).includes('bookmaru')","StudyVillageStudentQuestionOverrides?.refresh?.()","선생님이 지금 책마루에 열어 둔 문제가 없어요","_sourceNumber:i+1","overrides[`${base._sourceActivityId}:${base._sourceNumber}`]"] )assert.ok(library.includes(token),`Bookmaru metadata routing missing ${token}`);
