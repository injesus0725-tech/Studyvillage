/* v0.9.42 teacher activity open/close controls.
   Reads and writes use short timeouts, and repeated clicks for the same activity are ignored while a change is in flight. */
(()=>{
  ...
  const REQUEST_TIMEOUT_MS=5000,pending=new Set();
  ...
  list.addEventListener('click',async e=>{const b=e.target.closest('button[data-id]');if(!b)return;const id=b.dataset.id;if(pending.has(id))return;const name=b.dataset.name,currentlyOpen=b.dataset.open==='1',nextOpen=!currentlyOpen;if(currentlyOpen&&!confirm(`${name}의 문을 닫을까요?\n진행 중이던 임시 기록은 바로 삭제하지 않지만 학생은 다시 들어갈 수 없게 됩니다.`))return;pending.add(id);b.disabled=true;const original=b.textContent;b.textContent='처리 중…';try{const r=await timedFetch(`/api/admin/activity-state/${encodeURIComponent(id)}`,{method:'PUT',headers:{...headers(),'Content-Type':'application/json'},body:JSON.stringify({name,open:nextOpen})});if(!r.ok)throw new Error();await load()}catch{alert('활동 상태를 바꾸지 못했습니다. 서버 연결을 확인한 뒤 다시 시도해 주세요.');b.textContent=original}finally{pending.delete(id);b.disabled=false;load()}});
