...
assert.ok(/if\s*\(\s*name\.length\s*>\s*12\s*\)\s*return\s+res\.status\(400\)\.json\(\{\s*ok\s*:\s*false\s*,\s*code\s*:\s*'invalid-input'\s*\}\)/.test(guard),'overlong normalized student names must be rejected before the main login handler');
