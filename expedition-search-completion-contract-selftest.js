...
assert.ok(discovery.includes('host.dataset.startX')&&discovery.includes('host.dataset.startY'),'hidden discovery must account for randomized expedition start');
assert.ok(discovery.includes('Math.hypot(x-start.x,y-start.y)>.34'),'hidden discovery should require meaningful walking away from the spawn');
assert.ok(discovery.includes("host.querySelectorAll('.sv-stage-npc')")&&discovery.includes('>.16'),'hidden discovery should avoid spawning directly on NPCs');
