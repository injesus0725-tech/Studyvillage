...
assert.ok(guide.includes('overlay.hidden=true')&&guide.includes("b.addEventListener('click',openGuide)"),'new student login must remain nonblocking and onboarding must be manually opened');
...
assert.ok(server.includes('availableIds.has(requested)')&&server.includes("UPDATE players SET base_character=?"),'the server must validate ownership and persist base character changes');
