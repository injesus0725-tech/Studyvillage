...
assert.ok(snapshot.includes('return{type,word:'),'문제 수정 이력 스냅샷은 허용된 필드만 명시적으로 구성해야 합니다.');
...
assert.ok(snapshot.includes('acceptedAnswers:Array.isArray(q.acceptedAnswers)?q.acceptedAnswers.slice(0,8)'),'입력형 인정 정답도 제한된 배열로만 기록되어야 합니다.');
