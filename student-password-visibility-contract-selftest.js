...
const localStorageWrites=[...auth.matchAll(/localStorage\.setItem\(([^;]+)\);/g)].map(match=>match[1]);
assert.ok(localStorageWrites.every(write=>!/(?:^|[,{]\s*)password\s*(?:[,}:]|$)/i.test(write)),'평문 비밀번호를 localStorage에 저장하면 안 됩니다.');
assert.ok(auth.includes('passwordHash'),'로컬 계정 비밀번호는 해시 형태로 저장되어야 합니다.');
