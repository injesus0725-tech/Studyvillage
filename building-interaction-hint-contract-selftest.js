...
const throttle=src.match(/if\(now-lastCheck>=(\d+)\)/);assert.ok(throttle&&Number(throttle[1])>=160&&Number(throttle[1])<=500,'building proximity geometry checks must be throttled to a tablet-safe bounded interval');
