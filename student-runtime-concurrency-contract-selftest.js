...
assert.ok(records.includes('let loadPromise=null'),'학생 기록 조회는 중복 요청 방지 Promise가 필요합니다.');
assert.ok(records.includes('if(loadPromise)return loadPromise'),'학생 기록 조회가 겹치면 기존 요청을 재사용해야 합니다.');
...
assert.ok(customize.includes('if(polling||document.hidden||!navigator.onLine'),'꾸미기 폴링은 중복·백그라운드·오프라인 상태를 건너뛰어야 합니다.');
