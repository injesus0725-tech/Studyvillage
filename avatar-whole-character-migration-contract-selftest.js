...
for(const slot of ['face','expression','hair','hat','glasses','outfit','bottom','shoes','bag','hand','pet'])assert.ok(customize.includes("'"+slot+"'"),`착용 슬롯 누락: ${slot}`);
for(const slot of ['face','expression','hair','hat','glasses'])assert.ok(studentShop.includes(`data-shop-slot="${slot}"`),`학생 상점에서 ${slot} 슬롯을 선택할 수 있어야 합니다.`);
