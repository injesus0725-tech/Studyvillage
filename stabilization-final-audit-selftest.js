...
// Student controls: one deterministic pointer path at home and one in expedition.
assert.ok(index.includes('가고 싶은 곳을 터치하거나 클릭하세요'),'student UI must explain pointer navigation');
assert.ok(index.includes('student-direct-movement.js?v=20260819a'),'home direct movement must load explicitly');
assert.ok(index.indexOf('game.js')<index.indexOf('student-direct-movement.js'),'home movement must load after canonical game helpers');
