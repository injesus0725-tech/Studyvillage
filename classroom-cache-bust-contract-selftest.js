...
assert.ok(server.includes("const BUILD_ID='20260824-cachefix1'"),'the packaged classroom server must expose an unmistakable build id');
...
assert.ok(game.includes("replaceChildren('복구빌드 R2 · 20260824')"),'student login screen must show the unique recovery build marker');
assert.ok(admin.includes("marker.textContent='복구빌드 R2 · 20260824'"),'teacher screen must show the unique recovery build marker');
