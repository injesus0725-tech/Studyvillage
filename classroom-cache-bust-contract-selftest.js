...
assert.ok(server.includes("const BUILD_ID='20260825-r5-xp-balance'"),'the packaged classroom server must expose an unmistakable build id');
...
assert.ok(game.includes("replaceChildren('균형조정 R5 · 20260825')"),'student login screen must show the unique recovery build marker');
assert.ok(admin.includes("marker.textContent='균형조정 R5 · 20260825'"),'teacher screen must show the unique recovery build marker');
