...
assert.ok(server.includes("const BUILD_ID='20260825-r6-activity-return'"),'the packaged classroom server must expose an unmistakable build id');
...
assert.ok(game.includes("replaceChildren('활동복귀 R6 · 20260825')"),'student login screen must show the unique recovery build marker');
assert.ok(admin.includes("marker.textContent='활동복귀 R6 · 20260825'"),'teacher screen must show the unique recovery build marker');
