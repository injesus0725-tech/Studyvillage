...
assert.ok(server.includes("const BUILD_ID='20260824-r4-integrated-save-results'"),'the packaged classroom server must expose an unmistakable build id');
...
assert.ok(game.includes("replaceChildren('통합복구 R4 · 20260824')"),'student login screen must show the unique recovery build marker');
assert.ok(admin.includes("marker.textContent='통합복구 R4 · 20260824'"),'teacher screen must show the unique recovery build marker');
