...
for(const [slot,label] of [['all','전체'],['character','완전체 캐릭터'],['effect','효과'],['pet','펫'],['physical','실물']])assert.ok(src.includes(`data-shop-slot="${slot}"`),`${label} 상점 필터가 필요합니다.`);
for(const retired of ['hair','outfit','face','expression','hat','glasses','bottom','shoes','bag','hand'])assert.ok(!src.includes(`data-shop-slot="${retired}"`),`폐기된 ${retired} 상점 필터가 남아 있으면 안 됩니다.`);
...
assert.ok(src.includes('b.disabled=bought||limitReached||!available||salePending||saleEnded||!levelOk||!affordable||busy'),'필터 후에도 구매 한도를 포함한 안전 조건을 유지해야 합니다.');
assert.ok(src.includes("item.slot==='character'"),'완전체 캐릭터 상품 미리보기가 필요합니다.');
