...
const activityPostAt=source.indexOf("app.post('/api/player/me/activity'");
const activityPostSource=activityPostAt<0?'':source.slice(activityPostAt);
const cachedAt=activityPostSource.indexOf('const name=req.session.name,cached=cachedSubmission');
const openDbAt=activityPostSource.indexOf('db=openDb()');
