...
assert.ok(src.includes("관리자 로그인 시간이 만료되었습니다. 다시 로그인한 뒤 저장해 주세요."),'상점 설정 저장 중 401이면 재로그인 안내를 표시해야 합니다.');
assert.ok(src.includes("finally{loading=false}"),'상점 설정 조회 종료 후 로딩 잠금을 해제해야 합니다.');
assert.ok(src.includes("finally{setSaving(false)}"),'상점 설정 저장 종료 후 저장 잠금을 해제해야 합니다.');
