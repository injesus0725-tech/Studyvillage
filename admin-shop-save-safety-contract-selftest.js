...
assert.ok(/installItemShopRoutes\(app,\s*\{\s*requireSession\s*,\s*requireAdmin\s*,\s*publishLiveEvent\s*\}\s*\);/.test(studentRoutes),'학생 상점 라우트는 학생 세션 인증과 제한된 축하 알림 발행기를 포함해 연결되어야 합니다.');
assert.ok(adminRoutes.includes("import { installItemShopRoutes } from './item-shop.js';"),'교사 상점 라우트 연결 import가 필요합니다.');
assert.ok(/installItemShopRoutes\(app,\s*\{\s*requireAdmin\s*\}\s*\);/.test(adminRoutes),'교사 상점 라우트는 관리자 인증으로 연결되어야 합니다.');
