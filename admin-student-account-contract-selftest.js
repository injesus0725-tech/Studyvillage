...
const checks=[
  ...
  [html.indexOf('assets/admin-runtime-fixes.js')>html.indexOf('admin-student-edit.js'),'안정화 캡처 가드는 기본 학생 편집 처리기 뒤에 로드되어야 합니다.'],
  [runtime.includes("document.addEventListener('click'")&&runtime.includes('event.stopImmediatePropagation()')&&runtime.includes('},true)'),'안정화 캡처 가드는 학생 편집 입력을 document capture 단계에서 단일 처리해 중복 쓰기를 막아야 합니다.']
];
...
if(failures.length){
  ...
  console.log('admin student account stabilized single-write contract self-test passed');
}
