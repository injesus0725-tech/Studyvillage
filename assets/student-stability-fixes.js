...
(()=>{
  ...
  async function refresh(){if(ranking.hidden)return;const id=++requestId,me=document.querySelector('#profile-name')?.textContent?.trim();list.innerHTML='<p class="activity-summary-empty">랭킹을 불러오는 중이에요.</p>';try{const players=sortPlayers(await window.StudyVillageData.listPlayers());if(id!==requestId||ranking.hidden)return;list.innerHTML='';if(!players.length){list.innerHTML='<p class="activity-summary-empty">아직 랭킹 기록이 없어요.</p>';return}players.slice(0,30).forEach((p,i)=>{const row=document.createElement('article'),equipment=p.equipment&&typeof p.equipment==='object'?p.equipment:{},title=String(p.title||'새싹 주민');row.className=`sv-rank-row${p.name===me?' me':''}`;row.innerHTML=`<span class="sv-rank-number">${i<3?['🥇','🥈','🥉'][i]:i+1}</span><span class="sv-rank-avatar"><span class="sv-rank-hair"></span><span class="sv-rank-outfit"></span><span class="sv-rank-shoes"></span><span class="sv-rank-base"></span><span class="sv-rank-hat"></span><span class="sv-rank-glasses"></span><span class="sv-rank-bag"></span><span class="sv-rank-hand"></span><span class="sv-rank-pet"></span></span><span class="sv-rank-name"><b class="sv-rank-level">Lv.${Number(p.level)||1}</b><span class="sv-rank-title" title="${esc(title)}">🏷️ ${esc(title)}</span><strong>${esc(p.name)}${p.name===me?' · 나':''}</strong></span><span class="sv-rank-metric">${metricLabel(p)}</span>`;list.appendChild(row);const r=window.StudyVillageAvatar;r?.paintBase(row.querySelector('.sv-rank-base'),p.baseCharacter||'student-default');for(const slot of ['hair','outfit','shoes','hat','glasses','bag','hand','pet'])r?.paintItem(row.querySelector(`.sv-rank-${slot}`),equipment[slot])})}catch{if(id===requestId)list.innerHTML='<p class="activity-summary-empty">랭킹을 불러오지 못했어요. 잠시 후 다시 눌러 주세요.</p>'}}
