/* Student tablet stability fixes: direct quiz launch, immediate records, and detailed ranking tabs. */
(()=>{
  const game=document.querySelector('#game-screen');if(!game)return;

  /* 1) Record panel should open immediately; server refresh can finish afterward. */
  ...
  /* 2) Quiz hall: open the existing riddle quiz while the player is still beside the hall,
        then close only the building interior. This avoids the old leave -> delayed fake Space race. */
  document.addEventListener('click',event=>{
    ...
    setTimeout(()=>document.querySelector('#interior-exit')?.click(),0);
  },true);

  /* 3) Detailed classroom ranking tabs. */
  const ranking=document.querySelector('#student-ranking-panel');
  const rankingButton=document.querySelector('.sv-quick-button.ranking');
  if(!ranking||!rankingButton)return;
  const card=ranking.querySelector('.sv-hub-card'),list=ranking.querySelector('#student-ranking-list');
  if(!card||!list)return;
  const tabs=document.createElement('div');tabs.className='sv-ranking-tabs';
  const modes=[
    ['level','레벨'],['totalScore','누적 점수'],['bestScore','최고 점수'],['attempts','도전 횟수']
  ];
  let mode='level',requestId=0;
  for(const [key,label] of modes){const b=document.createElement('button');b.type='button';b.dataset.rankMode=key;b.textContent=label;b.className=key===mode?'selected':'';tabs.appendChild(b)}
  ranking.querySelector('.sv-hub-head')?.after(tabs);
  const style=document.createElement('style');style.textContent=`
    .sv-ranking-tabs{display:flex;gap:7px;flex-wrap:wrap;margin:14px 0 4px}.sv-ranking-tabs button{border:0;border-radius:999px;padding:8px 11px;background:#edf3e9;color:#56705c;font-weight:900;cursor:pointer}.sv-ranking-tabs button.selected{background:#4f8c5b;color:white}.sv-rank-metric{justify-self:end;font-weight:1000;color:#6b5520;background:#fff1bd;border-radius:999px;padding:5px 8px;white-space:nowrap}.sv-rank-row{grid-template-columns:42px 72px minmax(0,1fr) auto!important}@media(max-width:700px){.sv-rank-row{grid-template-columns:38px 58px minmax(0,1fr)!important}.sv-rank-metric{grid-column:3;justify-self:start}}
  ...
  const sortPlayers=players=>players.sort((a,b)=>{
    const primary=metricValue(b)-metricValue(a);if(primary)return primary;
    const xp=(Number(b.xp)||0)-(Number(a.xp)||0);if(xp)return xp;
  ...
  tabs.addEventListener('click',event=>{const b=event.target.closest('button[data-rank-mode]');if(!b)return;mode=b.dataset.rankMode;tabs.querySelectorAll('button').forEach(x=>x.classList.toggle('selected',x===b));refresh()});
  rankingButton.addEventListener('click',()=>setTimeout(refresh,0));
  window.addEventListener('studyvillage:ranking-refresh',()=>setTimeout(refresh,0));
})();
