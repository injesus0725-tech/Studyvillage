...
(()=>{
  ...
  function prepare(){
    ...
    /* MutationObserver watches this stage. Rewriting gate.innerHTML on every observation created
       a self-triggering mutation loop on tablets, which made the exploration appear frozen. */
    if(gate.dataset.touchPrepared!=='1'){
      gate.dataset.touchPrepared='1';
    ...
    if(player.dataset.touchPrepared!=='1'){
      player.dataset.touchPrepared='1';player.style.transition='left .45s ease, top .45s ease';player.style.bottom='auto';player.style.top='68%';
    }
  ...
  /* A direct gate tap belongs to the expedition engine. Never swallow that event here. */
  stage.addEventListener('click',event=>{
    const gateButton=event.target.closest('.sv-stage-npc');if(gateButton)return;
