/* Expedition entry/result safety guard.
   ...
   A transition lock prevents repeated taps from starting overlapping sessions, and authoritative attempt-limit save rejections are terminal. */
(()=>{
  ...
  function normalizeTerminalResult(stage){
    ...
    button.onclick=()=>{
      ...
      document.querySelector('.sv-quick-button.explore,.record-button')?.focus?.();
    };
  }
  function installResultGuard(){
  ...
  if(!installResultGuard()){
    const bootObserver=new MutationObserver(()=>{if(installResultGuard())bootObserver.disconnect()});
    bootObserver.observe(document.body,{subtree:true,childList:true});
  }
})();
