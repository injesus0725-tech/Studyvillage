/* Village math-playground wrong-answer review helper. It never changes the requested math mode: the village playground keeps the server's mixed random review, while exploration sends its own explicit mode. */
(()=>{
  if(window.StudyVillageMathReview)return;
  let latest=[];const originalFetch=window.fetch.bind(window);
  window.fetch=async(input,options={})=>{const url=String(input||'');const response=await originalFetch(input,options);if(/\/math-practice\/[^/]+\/answers(?:\?|$)/.test(url)&&document.querySelector('.math-practice-panel:not([hidden])')){try{const data=await response.clone().json();if(data.ok&&Array.isArray(data.review))latest=data.review}catch{}}return response};
