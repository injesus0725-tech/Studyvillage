/* Expedition variety: keep the original verified NPC challenge and add 1-2 randomized guides with distinct pre-question styles. */
(()=>{
  ...
  function rebuild(){
    ...
    const selected=shuffle(roster).slice(0,1+Math.floor(Math.random()*2)),spots=shuffle([{x:.66,y:.18},{x:.80,y:.42},{x:.62,y:.68},{x:.34,y:.20},{x:.82,y:.72}]).slice(0,selected.length);
    selected.forEach((npc,i)=>{const spot=spots[i],el=document.createElement('button');el.type='button';el.className='sv-stage-npc';el.dataset.varietyClone='1';el.dataset.varietyNpc=npc.id;el.style.left=`${Math.round(spot.x*100)}%`;el.style.top=`${Math.round(spot.y*100)}%`;el.innerHTML=`<span class="ico">${npc.icon}</span><strong>${npc.name}</strong><small>${npc.trait} · 문제 도전</small>`;el.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();runBehavior(npc,()=>original.click())});map.appendChild(el)});
    toast(`🧭 이번 지역에는 문제를 내는 친구가 ${1+selected.length}명 있어요.`);
  }
  let timer=0;new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(rebuild,100)}).observe(stage,{attributes:true,attributeFilter:['hidden'],childList:true,subtree:true});window.addEventListener('studyvillage:session-cleared',()=>stage.querySelectorAll('[data-variety-clone]').forEach(el=>el.remove()));
})();
