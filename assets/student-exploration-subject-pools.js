...
(()=>{
  ...
  function pool(group,count=5){const source=rows().filter(eligible).filter(q=>group==='integrated'?INTEGRATED.has(q.subject):q.subject===group);return window.StudyVillageQuestionResponse?.balancedSample?.(source,count)||shuffle(source).slice(0,count)}
