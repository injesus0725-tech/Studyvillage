...
(()=>{
  ...
  const items=[
    ['학생 현황',()=>[...app.querySelectorAll('.panel')].find(p=>p.querySelector('h2')?.textContent.includes('학생 성장 현황'))],
    ['활동 도전',()=>document.querySelector('#attempt-policy-panel')],
    ['별 관리',()=>document.querySelector('#admin-star-panel')],
    ['상점',()=>document.querySelector('#shop-admin-panel')],
    ['오류 기록',()=>[...app.querySelectorAll('.panel')].find(p=>/오류|진단/.test(p.querySelector('h2')?.textContent||''))],
    ['백업·복원',()=>[...app.querySelectorAll('.panel')].find(p=>p.querySelector('h2')?.textContent.includes('백업과 복원'))]
  ];
