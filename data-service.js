/* v0.9.32 data service. Classroom server is the source of truth; local storage is fallback only.
   ...
   savePlayerConfirmed is for activities that must keep a checkpoint until the classroom server explicitly confirms the save. */
window.StudyVillageData=(()=>{
  ...
  async function savePlayerConfirmed(record){
    ...
    try{const r=await timedFetch('/api/player/me/record',{method:'POST',headers:{'Content-Type':'application/json',...window.StudyVillageAuth.authHeaders()},body:JSON.stringify(payload)});if(!r.ok)return null;return{...empty(),...((await r.json()).player||{})}}catch{return null}
  }
  async function savePlayer(name,record){
    const confirmed=await savePlayerConfirmed(record);if(confirmed)return confirmed;
    const payload={totalScore:Number(record.totalScore)||0,attempts:Number(record.attempts)||0,bestScore:Number(record.bestScore)||0,lastScore:Number(record.lastScore)||0};
  ...
  return{loadPlayer,savePlayer,savePlayerConfirmed,listPlayers,mode:async()=>await ready()?'classroom-server':'local'}
})();