...
assert.ok(nav.includes('app.prepend(sticky)'),'sticky controls must start at the top of the admin screen');
assert.ok(nav.includes('if(app.firstElementChild!==sticky)app.prepend(sticky)'),'late admin panels must not push sticky controls below delivery requests');
assert.ok(nav.includes('new MutationObserver(keepStickyFirst).observe(app,{childList:true})'),'sticky controls must recover after dynamic panel insertion');
...
assert.ok(html.includes('admin-delivery-notifications.js?v=20260909sticky1')&&html.includes('admin-quick-navigation.js?v=20260910sticky5'),'admin sticky controls must use fresh cache keys');
