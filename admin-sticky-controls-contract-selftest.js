...
assert.ok(nav.includes('.admin-sticky-controls{position:fixed;top:8px'),'main admin controls must remain visible while scrolling');
...
assert.ok(nav.includes("app.classList.add('admin-has-fixed-controls');app.appendChild(sticky)"),'fixed controls must not reorder dynamic admin panels');
assert.ok(!nav.includes('new MutationObserver(keepStickyFirst)'),'fixed controls must not create a DOM reorder loop');
...
assert.ok(html.includes('admin-delivery-notifications.js?v=20260909sticky1')&&html.includes('admin-quick-navigation.js?v=20260910fixed6'),'admin sticky controls must use fresh cache keys');
