...
assert.ok(shop.includes("REMOVED_STYLE_SLOTS=new Set(['face','expression','hat','glasses'])"),'retired face and head-accessory slots must be excluded from the live catalog');
for(const slot of ['hair','outfit','bottom','shoes','hand']){assert.ok(index.includes(`id="player-${slot}"`)&&index.includes(`id="preview-${slot}"`),`${slot} layers must exist in village and preview`)}
for(const id of ['hair-short','hair-bob','hair-ponytail','hair-blue','outfit-hoodie','outfit-uniform','outfit-wizard','outfit-armor','bottom-jeans','bottom-shorts','bottom-skirt','shoes-sneakers','shoes-boots','shoes-wing','hand-sword','hand-wand','hand-book','hand-magnifier','pet-dog','pet-rabbit','pet-dragon','pet-slime'])assert.ok(shop.includes(`'${id}'`)&&renderer.includes(`'${id}'`),`expanded item missing from shop or renderer: ${id}`);
