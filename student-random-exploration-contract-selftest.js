...
for(const name of ['국어의 숲','전체 문제','곱셈·나눗셈','사회·과학·예체능 탐험'])assert.ok(hub.includes(`name:'${name}'`),`missing active exploration ${name}`);
...
assert.strictEqual((hub.match(/\{id:'(?:korean|math-all|math-mul-div|integrated)'/g)||[]).length,4,'exploration hub must expose exactly the four requested exploration cards');
