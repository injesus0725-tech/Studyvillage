...
assert.match(client,/npcCount\+eventCount\+locationCount/,'overview must calculate collection progress from all three categories');
assert.match(client,/total===catalogTotal\?'🏆 도감 완성'/,'completed collections must be easy to identify');
assert.match(client,/학교 장소 \$\{locationCount\} \/ 9/,'teacher overview must show school location progress');
