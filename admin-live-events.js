...
(()=>{
  ...
  const DRAFT_KEY='studyvillage-admin-live-draft:v1';
  ...
  async function publish(message,icon='✨',{clearDraftOnSuccess=true}={}){
    ...
    try{
      ...
      lastSentAt=Date.now();result.textContent=(d.recipients||0)>0?`현재 접속 ${d.recipients}명에게 전송 ✓`:'현재 접속 중인 학생이 없어 전송 대상이 없습니다.';
      if(clearDraftOnSuccess&&d.recipients>0)clearDraft();
      refreshAudience();
    }catch{result.textContent='방송을 보내지 못했습니다.'}finally{setBusy(false)}
  }
  send.onclick=()=>publish(input.value,'📣',{clearDraftOnSuccess:true});
  test.onclick=()=>publish('실시간 마을 알림이 정상적으로 연결되었습니다!','⭐',{clearDraftOnSuccess:false});
