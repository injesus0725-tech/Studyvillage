...
assert.ok(index.includes('sound-effects.js?v=20260825v1'),'student page must load the V1 sound manager');
assert.ok(index.indexOf('sound-effects.js')<index.indexOf('game.js?v=20260825v1'),'sound manager must load before activities');
for(const token of ['studyvillage-sound-enabled-v1',"button.id='sound-toggle'",'correct:', 'wrong:', 'reward:', 'danger:', 'complete:', 'levelup:'])assert.ok(sound.includes(token),`sound manager missing ${token}`);
