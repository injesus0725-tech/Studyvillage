...
for(const [slot,label] of [['all','전체'],['character','완전체 캐릭터'],['effect','효과'],['pet','펫'],['physical','실물']])assert.ok(src.includes(`data-admin-shop-slot="${slot}"`),`${label} 관리자 상점 필터가 필요합니다.`);
assert.ok(src.includes("[data-admin-shop-slot=\"hair\"]')?.remove()")&&src.includes("[data-admin-shop-slot=\"outfit\"]')?.remove()"),'관리자 화면에서 분리형 머리·복장 필터를 제거해야 합니다.');
for(const retired of ['hat','glasses','bottom','shoes','bag','hand'])assert.ok(!src.includes(`data-admin-shop-slot="${retired}"`),`폐기된 ${retired} 관리자 필터를 다시 표시하면 안 됩니다.`);
