...
for(const [slot,label] of [['all','전체'],['hat','모자'],['glasses','안경'],['bag','가방'],['pet','친구']])assert.ok(src.includes(`data-admin-shop-slot="${slot}"`),`${label} 관리자 상점 필터가 필요합니다.`);
...
assert.ok(src.includes('applyFilter();dirty=false;const onSale='),'서버에서 다시 불러온 뒤에도 현재 필터를 유지해야 합니다.');
