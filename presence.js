/* v0.9.56 lightweight student presence heartbeat.
   Heartbeats time out quickly and run only while the authenticated student game is visible. */
(()=>{
  const game=document.querySelector('#game-screen');
  const HEARTBEAT_MS=15000,INTERACTION_COOLDOWN_MS=8000,REQUEST_TIMEOUT_MS=4000;let timer=null,lastPing=0,pinging=false,authExpired=false;
  function active(){return !authExpired&&!document.hidden&&navigator.onLine&&(!game||game.classList.contains('active'))}
  async function ping(force=false){const headers=window.StudyVillageAuth?.authHeaders?.()||{};if(!headers.Authorization||!active()||pinging)return;const now=Date.now();if(!force&&now-lastPing<INTERACTION_COOLDOWN_MS)return;pinging=true;const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),REQUEST_TIMEOUT_MS);try{const r=await fetch('/api/presence/heartbeat',{method:'POST',headers,cache:'no-store',signal:controller.signal});if(r.status===401){authExpired=true;stop();window.StudyVillageAuth?.clearSession?.();return}if(r.ok)lastPing=Date.now()}catch{}finally{clearTimeout(timeout);pinging=false}}
  ...
  document.addEventListener('visibilitychange',()=>{if(document.hidden)stop();else start()});window.addEventListener('online',start);window.addEventListener('offline',stop);window.addEventListener('focus',()=>{start();ping(true)});window.addEventListener('pointerdown',activity,{passive:true});window.addEventListener('keydown',activity,{passive:true});window.addEventListener('studyvillage:session-ready',()=>{authExpired=false;start()});window.addEventListener('studyvillage:session-cleared',()=>{authExpired=true;stop()});
