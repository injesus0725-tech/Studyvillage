/* v1.10 classroom backup download contract. */
...
for(const token of ["/api/admin/backup","backupButton.disabled=true","await r.blob()","Content-Disposition","a.download=filename","URL.revokeObjectURL(url)"])assert.ok(source.includes(token),`admin.js: missing backup download step ${token}`);
...
assert.ok(guard.includes('관리자 로그인이 만료되었습니다.'),'세션 만료 이유를 교사에게 알려야 합니다.');
