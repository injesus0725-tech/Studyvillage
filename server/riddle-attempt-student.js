...
const levelFromXp=xp=>Math.max(1,Math.floor((Number(xp)||0)/200)+1),xpIntoLevel=xp=>(Number(xp)||0)%200;
...
function playerView(r){const rewards=rewardsFor(r);return{name:r.name,totalScore:r.total_score,attempts:r.attempts,bestScore:r.best_score,lastScore:r.last_score,loginCount:r.login_count||0,lastLoginAt:r.last_login_at||null,xp:r.xp||0,level:levelFromXp(r.xp),xpIntoLevel:xpIntoLevel(r.xp),xpToNext:200,title:rewards.title,badges:rewards.badges,updatedAt:r.updated_at}}
