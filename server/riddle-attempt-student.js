  ...
  app.post('/api/player/me/record',requireSession,(req,res)=>{
    ...
    try{
      ...
      const tx=db.transaction(()=>{
        ...
        return{player:playerView(db,db.prepare('SELECT * FROM players WHERE name=?').get(name)),gainedXp:gained,extraAttemptsUsed:extraNeeded,extraAttemptsRemaining:Math.max(0,latestExtra-extraNeeded)};
      });
