...
for(const effectId of ['effect-snow-aura','effect-music-aura','effect-rainbow-aura']){
  const newEffectCustomization=structuredClone(currentCustomization);
  newEffectCustomization.players[0].equipment_json=JSON.stringify({effect:effectId});
  newEffectCustomization.players[0].owned_items_json=JSON.stringify([effectId]);
  assert.equal(prepareStudyvillageRestore(newEffectCustomization).ok,true,`${effectId} must remain valid through backup restore`);
}

for(const characterId of ['character-ice-mage','character-flower-princess','character-sea-princess']){
...
for(const petId of ['pet-shih-tzu','pet-black-cat','pet-baby-penguin']){
...
for(const effectId of ['effect-daydream-aura','effect-focus-aura','effect-bunny-aura']){
  const newEffectCustomization=structuredClone(currentCustomization);
  newEffectCustomization.players[0].equipment_json=JSON.stringify({effect:effectId});
  newEffectCustomization.players[0].owned_items_json=JSON.stringify([effectId]);
  assert.equal(prepareStudyvillageRestore(newEffectCustomization).ok,true,`${effectId} must remain valid through backup restore`);
...
for(const characterId of ['character-desert-princess','character-brave-warrior','character-little-witch']){
  const newCharacterCustomization=structuredClone(base);
  newCharacterCustomization.players[0].base_character=characterId;
  newCharacterCustomization.players[0].owned_items_json=JSON.stringify([characterId]);
  assert.equal(prepareStudyvillageRestore(newCharacterCustomization).ok,true,`${characterId} must remain valid through backup restore`);
...
for(const effectId of ['effect-surprise-aura','effect-sprout-aura','effect-ghost-aura']){
  const newEffectCustomization=structuredClone(currentCustomization);
  newEffectCustomization.players[0].equipment_json=JSON.stringify({effect:effectId});
  newEffectCustomization.players[0].owned_items_json=JSON.stringify([effectId]);
  assert.equal(prepareStudyvillageRestore(newEffectCustomization).ok,true,`${effectId} must remain valid through backup restore`);
...
for(const characterId of ['character-royal-prince','character-star-wizard','character-ice-mage-boy']){
  const newCharacterCustomization=structuredClone(base);
  newCharacterCustomization.players[0].base_character=characterId;
  newCharacterCustomization.players[0].owned_items_json=JSON.stringify([characterId]);
  assert.equal(prepareStudyvillageRestore(newCharacterCustomization).ok,true,`${characterId} must remain valid through backup restore`);
...
for(const petId of ['pet-dachshund','pet-calico-cat','pet-phoenix']){
  const newPetCustomization=structuredClone(currentCustomization);
  newPetCustomization.players[0].equipment_json=JSON.stringify({pet:petId});
  newPetCustomization.players[0].owned_items_json=JSON.stringify([petId]);
  assert.equal(prepareStudyvillageRestore(newPetCustomization).ok,true,`${petId} must remain valid through backup restore`);
...
for(const effectId of ['effect-crown-aura','effect-sweat-aura','effect-anger-aura']){
  const newEffectCustomization=structuredClone(currentCustomization);
  newEffectCustomization.players[0].equipment_json=JSON.stringify({effect:effectId});
  newEffectCustomization.players[0].owned_items_json=JSON.stringify([effectId]);
  assert.equal(prepareStudyvillageRestore(newEffectCustomization).ok,true,`${effectId} must remain valid through backup restore`);
