...
const MODE_GENERATORS=Object.freeze({mixed:GENERATORS,addition:[addition],subtraction:[subtraction],multiplication:[multiplication],division:[division],fraction:[fraction,unitFraction,decimal]});
function normalizeMode(value){const mode=clean(value,30).toLowerCase();return mode in MODE_GENERATORS?mode:'mixed'}
function makeProblem(mode='mixed'){return pick(MODE_GENERATORS[normalizeMode(mode)])()}
export function installMathPracticeRoutes(app,{requireSession}){
  app.post('/api/player/me/math-practice/start',requireSession,(req,res)=>{prune();const mode=normalizeMode(req.body?.mode),id=crypto.randomUUID(),problems=Array.from({length:5},()=>makeProblem(mode));sessions.set(id,{id,name:req.session.name,mode,problems,createdAt:Date.now(),authorizedScore:null,finalized:false});res.json({ok:true,activityId:ACTIVITY_ID,mode,sessionId:id,problems:problems.map((problem,index)=>({id:index+1,prompt:problem.prompt,unit:problem.unit}))})});
