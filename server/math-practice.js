...
const EXPLORATION_RIDDLES=Object.freeze([
 ...
 {prompt:'다리는 네 개인데 걷지 못하는 것은?',options:['의자','강아지','말','고양이'],answer:0,explanation:'의자는 다리가 네 개지만 스스로 걷지 못해요.'},
 {prompt:'쓰면 쓸수록 작아지는 것은?',options:['연필','공책','가방','책상'],answer:0,explanation:'연필은 사용할수록 짧아져요.'},
 {prompt:'문은 문인데 열 수 없는 문은?',options:['소문','현관문','창문','교문'],answer:0,explanation:'소문은 이름에 문이 있지만 실제 문은 아니에요.'},
...
function answerText(problem){if(problem.answerFormat==='quotient-remainder')return`${problem.answer.q}, ${problem.answer.r}`;if(problem.answerFormat==='choice'||problem.type==='choice')return String(problem.options?.[problem.answer]??problem.answer);return String(problem.answer)}
