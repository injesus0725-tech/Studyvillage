...
try{
  ...
  assert.deepEqual(JSON.parse(cleaned.owned_items_json),['pet-maltese-production'],'retired pets must be removed while production pets remain');
  assert.equal(JSON.parse(cleaned.equipment_json).pet,null,'equipped retired pet must be cleared');
  assert.deepEqual(JSON.parse(cleanupDb.prepare('SELECT value FROM settings WHERE key=?').get(`compat:owned-items:${encodeURIComponent('펫정리학생')}`).value),['pet-maltese-production'],'legacy wardrobe mirror must not resurrect retired pets');
  cleanupDb.close();
