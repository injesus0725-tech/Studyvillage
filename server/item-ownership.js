...
export function validateOwnedItemsStrict(value){
  ...
  if(raw.length>MAX_OWNED_ITEMS)return{ok:false,code:'wardrobe-full',items:[]};
  const items=[],seen=new Set();
  for(const entry of raw){
    if(typeof entry!=='string'||!SAFE_ID.test(entry))return{ok:false,code:'invalid-item-id',items:[]};
    if(seen.has(entry))return{ok:false,code:'duplicate-item-id',items:[]};
    seen.add(entry);items.push(entry);
  }
