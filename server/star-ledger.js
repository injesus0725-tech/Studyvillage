...
export function explorationCollectionFor(playerName){const name=clean(playerName,12);if(!name)return null;let db;try{db=openLiveDb();if(!db.prepare('SELECT 1 FROM players WHERE name=?').get(name))return null;return{ok:true,...readCollection(db,name)}}finally{try{db?.close()}catch{}}}
...
const DAILY_MISSIONS=Object.freeze([
  {id:'forest',activityId:'exploration-forest-riddle',giver:'숲길 여우',icon:'🦊',title:'생각의 숲 한 바퀴',detail:'생각의 숲 탐험을 한 번 완료해요.',stars:3},
  {id:'mountain',activityId:'exploration-mountain-riddle',giver:'꼬마 용',icon:'🐲',title:'도전의 산 오르기',detail:'도전의 산 탐험을 한 번 완료해요.',stars:5},
  {id:'vocabulary',activityId:'vocabulary',giver:'책방 유령',icon:'👻',title:'책마루 낱말 수집',detail:'책마루 낱말 활동을 한 번 완료해요.',stars:3}
]);
const classroomDay=()=>new Date().toLocaleDateString('en-CA',{timeZone:'Asia/Seoul'});
function dailyMissionFor(name){const day=classroomDay(),seed=[...`${name}:${day}`].reduce((sum,char)=>sum+char.codePointAt(0),0);return{...DAILY_MISSIONS[seed%DAILY_MISSIONS.length],day}}
