/* v0.9.68 server-side Studyvillage backup validator.
   Kept isolated from server.js so restore validation can be hardened before it is wired into the destructive restore path. */
...
export function validateStudyvillageBackup(backup){
  ...
  for(const p of backup.players){
    ...
    if(!integer(p?.total_score??0,0,100000000)||!integer(p?.attempts??0,0,1000000)||!integer(p?.best_score??0,0,1000)||!integer(p?.last_score??0,0,1000)||!integer(p?.login_count??0,0,1000000)||!integer(p?.xp??0,0,1000000000))return fail('invalid-player-number','학생 점수 또는 XP 정보가 정상 범위를 벗어납니다.');
  ...
  for(const s of backup.settings){
    ...
    if(!key||key.length>200||settingKeys.has(key)||typeof s?.value!=='string'||s.value.length>200000)return fail('invalid-setting','설정 정보가 비어 있거나 중복 또는 손상되었습니다.');
    settingKeys.add(key);
  }

  for(const row of backup.activities||[]){
    if(!row||!names.has(String(row.player_name||''))||!text(String(row.type||''),100)||!text(String(row.created_at||''),80)||String(row.detail??'').length>2000)return fail('invalid-activity-log','최근 활동 기록에 존재하지 않는 학생 또는 손상된 항목이 있습니다.');
  }
  ...
  for(const row of backup.activityRecords||[]){
    ...
    if(!integer(row?.attempts,0,1000000)||!integer(row?.best_score,0,1000)||!integer(row?.last_score,0,1000)||!integer(row?.total_score,0,100000000))return fail('invalid-activity-score','학습 활동 점수 정보가 정상 범위를 벗어납니다.');
  ...
  for(const row of backup.errorReports||[]){
    ...
    if(String(row?.message??'').length>1800||String(row?.stack??'').length>5000||String(row?.extra_json??'').length>6000||String(row?.recent_events_json??'').length>12000)return fail('oversized-error-report','오류 기록 내용이 정상 범위를 벗어납니다.');
