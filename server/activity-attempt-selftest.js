...
assert.deepEqual(normalizeAttemptPolicy({mode:'once',xpMode:'every-attempt'}),{mode:'once',limit:1,xpMode:'first-completion'});
assert.deepEqual(evaluateAttempt({mode:'once'},{attempts:0}),{policy:{mode:'once',limit:1,xpMode:'first-completion'},attempts:0,remaining:1,allowed:true,awardXp:true,completed:false});
...
assert.deepEqual(validateAttemptPolicyMap({'riddle-demo':{mode:'limited',limit:2,xpMode:'every-attempt'}}),{ok:true,policies:{'riddle-demo':{mode:'limited',limit:2,xpMode:'every-attempt'}}});
