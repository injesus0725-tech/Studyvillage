...
assert.ok(src.includes('new MutationObserver(armNavigation)')&&src.includes('armNavigation();'),'back protection must start on the QR login screen and remain armed after entering the game');
assert.ok(src.includes('function armNavigation(){if(navigationArmed)return;'),'login-screen protection must not depend on the game already being active');
