...
try{
  ...
  result=await request('/api/admin/players',{token:adminToken});const restored=(result.data.players||[]).find(row=>row.name==='안정화학생2');assert.ok(restored);assert.equal(restored.xp,600,'backup/restore must preserve XP');
