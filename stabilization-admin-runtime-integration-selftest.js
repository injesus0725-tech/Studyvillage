...
try{
  ...
  result=await request('/api/player/me/activity',{method:'POST',token:studentToken,body:{activityId:'vocabulary',score:100,submissionId:'admin-runtime-vocabulary-0001'}});assert.equal(result.response.status,200);assert.equal(result.data.activityStars,2);
  result=await request('/api/admin/players',{token:adminToken});let liveStudent=(result.data.players||[]).find(row=>row.name==='안정화학생');assert.ok(liveStudent,'student must remain visible after vocabulary completion');assert.equal(liveStudent.xp,640,'perfect vocabulary completion must add 40 XP after teacher correction baseline');
