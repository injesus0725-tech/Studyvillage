...
assert.ok(/POLICY_ALIASES\s*=\s*\{\s*vocabulary\s*:\s*['"]library-vocabulary['"]\s*,\s*riddle\s*:\s*['"]riddle-demo['"]\s*\}/.test(server),'stored riddle and vocabulary IDs must resolve to teacher policy IDs');
assert.ok(/allowed\s*=\s*base\.allowed\s*\|\|\s*extra\s*>\s*0/.test(server),'server status must allow a teacher-granted extra attempt after the base limit is exhausted');
