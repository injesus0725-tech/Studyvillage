...
(()=>{
  ...
  function scheduleLocalLogin(){
    ...
    setTimeout(async()=>{
      ...
      try{const response=await previousFetch('/api/admin/local-session',{method:'POST',cache:'no-store'}),data=await response.json().catch(()=>({}));if(response.ok&&data.ok&&data.token){sessionStorage.setItem('studyvillage-admin-token',data.token);location.reload();return}}catch{}finally{localEntryRunning=false}
