...
(()=>{
  const marker=document.createElement('div');marker.textContent='활동복귀 R6 · 20260825';marker.style.cssText='position:fixed;right:8px;bottom:6px;z-index:99999;padding:4px 7px;border-radius:7px;background:#203f2e;color:#fff;font:700 11px system-ui';document.body.appendChild(marker);
  ...
  const passwordChange=document.querySelector('#change-admin-password');if(passwordChange){passwordChange.hidden=true;passwordChange.style.display='none'}const controls=document.createElement('script');controls.src='/assets/admin-direct-controls.js?v=20260825r10';controls.async=false;document.head.appendChild(controls);
