/* Stabilization: one student movement path. Tablet taps and PC mouse clicks move directly; keyboard/d-pad movement is ignored. */
(()=>{
  ...
  style.textContent=`.mobile-controls,.control-help{display:none!important}.sv-move-target{position:absolute;z-index:9;width:24px;height:24px;border:3px solid #fff;border-radius:50%;background:#5e9fff66;box-shadow:0 0 0 3px #2f78d655;transform:translate(-50%,-50%);pointer-events:none!important}@media(max-width:720px),(pointer:coarse){#world{touch-action:none!important}.interaction-hint{display:none!important}}`;
  ...
  function stop(){target=null;marker.hidden=true;if(raf){cancelAnimationFrame(raf);raf=0}try{state.keys?.clear?.()}catch{}}
  ...
  const blockKeyboard=event=>{if(game.classList.contains('active')&&movementKeys.has(event.key)){event.preventDefault();event.stopImmediatePropagation();try{state.keys?.clear?.()}catch{}}};
  window.addEventListener('keydown',blockKeyboard,true);window.addEventListener('keyup',blockKeyboard,true);
