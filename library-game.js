/* v0.8.4 Bookmaru vocabulary mini game + activity record */
(()=>{
  ...
  function open(){index=0;score=0;answered=false;saving=false;panel.hidden=false;render()}
  function render(){const item=questions[index];answered=false;progress.textContent=`${index+1} / ${questions.length}`;scoreEl.textContent=`${score}점`;word.textContent=item.word;feedback.textContent='';feedback.className='library-feedback';next.hidden=true;next.onclick=null;options.innerHTML='';item.options.forEach((text,i)=>{const b=document.createElement('button');b.className='library-option';b.textContent=`${i+1}. ${text}`;b.onclick=()=>answer(i);options.appendChild(b)})}
  ...
  async function saveActivity(){const r=await fetch('/api/player/me/activity',{method:'POST',headers:{'Content-Type':'application/json',...headers()},body:JSON.stringify({activityId:'vocabulary',score})});if(!r.ok)throw new Error('save-failed');return r.json()}
  async function finish(){if(saving)return;saving=true;progress.textContent='완료';word.textContent=score===100?'어휘 박사! 🏆':'책마루 도전 완료!';options.innerHTML='';feedback.textContent='기록 저장 중...';feedback.className='library-feedback';next.hidden=true;try{const result=await saveActivity();const record=result.record||{},xp=result.gainedXp||0;feedback.textContent=`5문제 중 ${score/20}문제 정답 · ${score}점 · +${xp} XP\n어휘 도전 ${record.attempts||1}회 · 최고 ${record.bestScore??score}점`;feedback.className='library-feedback success';window.dispatchEvent(new CustomEvent('studyvillage:library-complete',{detail:{score,correct:score/20,total:5,gainedXp:xp,record,player:result.player}}))}catch{feedback.textContent=`${score}점! 다만 기록 저장에 실패했어요. 서버 연결을 확인해 주세요.`;feedback.className='library-feedback error'}finally{saving=false;next.hidden=false;next.textContent='책마루로 돌아가기';next.onclick=()=>{panel.hidden=true;next.onclick=null}}}
