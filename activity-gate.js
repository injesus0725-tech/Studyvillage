...
(()=>{
  ...
  async function checkAttemptLimit(id,name){try{const headers=window.StudyVillageAuth?.authHeaders?.()||{},recordId=recordIds[id]||id,response=await timedFetch(`/api/player/me/activity-attempt-status/${encodeURIComponent(recordId)}`,{headers,cache:'no-store'});if(!response.ok)return{ok:true};const status=await response.json();if(status.ok&&status.allowed)return{ok:true,remaining:status.remaining,extraAttempts:status.extraAttempts};return{ok:false,message:`🔒 ${name}의 도전 횟수를 모두 사용했어요.\n선생님이 추가 도전을 허용하면 다시 참여할 수 있어요.`}}catch{return{ok:true}}}
