...
(()=>{
  ...
  function blockedByPanel(){return[...document.querySelectorAll('#dialogue,#quiz-panel,#record-panel,#customize-panel,#student-shop-panel,#library-game-panel,#library-game,#student-explore-panel,#study-expedition-stage,#curriculum-learning,.math-practice-panel,#building-interior')].some(el=>el&&!el.hidden&&el.getClientRects().length>0)}
  ...
  async function checkAttemptLimit(id,name){const unavailable={ok:false,message:`📡 ${name}의 남은 도전 횟수를 확인할 수 없어요.\n교실 서버 연결이 돌아오면 다시 시도해 주세요.`};try{const headers=window.StudyVillageAuth?.authHeaders?.()||{},recordId=recordIds[id]||id,response=await timedFetch(`/api/player/me/activity-attempt-status/${encodeURIComponent(recordId)}`,{headers,cache:'no-store'});if(!response.ok)return unavailable;const status=await response.json();if(!status.ok)return unavailable;const limit=Math.max(1,Number(status.policy?.limit)||1);if(status.allowed)return{ok:true,remaining:status.remaining,extraAttempts:status.extraAttempts,limit};return{ok:false,message:`🔒 ${name}의 오늘 ${limit}회와 추가 도전권을 모두 사용했어요.\n선생님이 추가 도전을 허용하면 다시 참여할 수 있어요.`}}catch{return unavailable}}
  async function allow(id,name){if(pending.has(id))return false;pending.add(id);try{const state=activityState();if(state){const open=await state.requireOpen(id,name);if(!open.ok){const now=Date.now();if(now-lastNoticeAt>=NOTICE_COOLDOWN_MS){lastNoticeAt=now;alert(open.message)}return false}}const attempts=await checkAttemptLimit(id,name);if(attempts.ok)return true;const now=Date.now();if(now-lastNoticeAt>=NOTICE_COOLDOWN_MS){lastNoticeAt=now;alert(attempts.message)}return false}finally{pending.delete(id)}}
