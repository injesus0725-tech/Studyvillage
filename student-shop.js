...
(()=>{
  ...
  let busy=false,loadPromise=null;
  ...
  function render(data={}){
    ...
    for(const item of data.items||[]){const bought=owned.has(item.id),available=item.available!==false,b=document.createElement('button');b.type='button';b.className=`inventory-item ${bought?'selected':''}`;b.disabled=bought||!available||busy;b.innerHTML=`<span>${icons[item.id]||'🎁'}</span><strong>${esc(item.name)}</strong><small>${bought?'구매 완료':available?`${Number(item.price)||0}별 · 구매`:'판매 중지'}</small>`;if(!bought&&available)b.onclick=()=>purchase(item);list.appendChild(b)}
  }
